...
contract Registry {
    ...
    // Thêm Mapping lưu trữ các Root đã được cấp
    mapping(address => mapping(bytes32 => bool)) public isIssued;
    ...
    // Thêm Event báo hiệu đã cấp bằng thành công
    event CredentialIssued(address indexed issuer, bytes32 indexed merkleRoot);
    ...
    function issueCredential(bytes32 _merkleRoot) external {
        if (!isAuthorizedIssuer[msg.sender]) revert NotAuthorizedIssuer();
        
        isIssued[msg.sender][_merkleRoot] = true;
        emit CredentialIssued(msg.sender, _merkleRoot);
    }
